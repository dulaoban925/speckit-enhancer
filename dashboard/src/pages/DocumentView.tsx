...
const DocumentView: React.FC = () => {
  ...
  const { document, loading, error, refreshDocument, updateMetadata, updateDocument } = useDocument(decodedPath)
  ...
  const handleLinkClick = (href: string) => {
    ...
    if (href.startsWith('./') || href.startsWith('../')) {
      const currentDir = document.relativePath.split('/').slice(0, -1).join('/')
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      console.log('[FRONTEND] Write response:', JSON.stringify(response, null, 2))
      console.log('[FRONTEND] response.success:', response.success, '(类型:', typeof response.success, ')')
      console.log('[FRONTEND] response.data:', response.data)
      console.log('[FRONTEND] response.error?.code:', response.error?.code)
      console.log('[FRONTEND] 条件判断: response.success && response.data =', response.success && response.data)
  ...
  const handleForceSave = useCallback(async () => {
    ...
    try {
      ...
      alert(`强制保存失败: ${error instanceof Error ? error.message : String(error)}`)
    } finally {
      setIsSaving(false)
    }
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-gh-canvas-default p-8">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-gh-danger-fg mb-2">加载失败</h2>
  ...
  if (!document) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gh-canvas-default p-8">
        <div className="text-center">
          <h2 className="text-2xl font-semibold text-gh-fg-default mb-2">文档未找到</h2>
          ...
          <nav className="flex items-center gap-2 text-sm mb-3">
            ...
            {breadcrumbs.map((crumb, index) => (
              <React.Fragment key={index}>
                ...
                {crumb.isLast ? (
                  <span className="text-gh-fg-default font-semibold">{crumb.name}</span>
                ) : (
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-gh-fg-muted">
                ...
                <span>最后修改: {formatDate(document.metadata.lastModified)}</span>
              </div>
            </div>
            ...
            <div className="flex-1">
              ...
              {conflictInfo && (
                <div className="mt-3 p-3 bg-gh-canvas-subtle rounded text-xs font-mono">
                  <div>预期修改时间: {new Date(conflictInfo.expectedMtime).toLocaleString('zh-CN')}</div>
                  <div>实际修改时间: {new Date(conflictInfo.actualMtime).toLocaleString('zh-CN')}</div>
                </div>
              )}
            </div>
          </div>
