...
1. **Given** 用户在 UI 中, **When** 用户按下 Cmd+K (Mac) 或 Ctrl+K (Windows/Linux), **Then** 应该打开全局搜索框 ✅
2. **Given** 用户在搜索框输入关键词, **When** 用户停止输入 300ms 后, **Then** 应该实时显示匹配的文档列表和匹配位置 ✅
3. **Given** 搜索返回结果, **When** 结果显示, **Then** 应该包含文档名、相对路径、匹配行号、匹配文本片段（带黄色高亮） ✅
4. **Given** 搜索返回结果, **When** 用户点击某个搜索结果, **Then** 应该跳转到对应文档、滚动到目标行、并高亮显示实际搜索匹配的文本 ✅
5. **Given** 文档中有多个相同关键词匹配, **When** 用户点击特定行的搜索结果, **Then** 系统应该智能定位到最接近目标行号的匹配项 ✅
6. **Given** 用户跳转到搜索结果, **When** 高亮显示后, **Then** 高亮应该永久保留直到用户导航到其他页面 ✅
7. **Given** 用户搜索某个关键词, **When** 系统查询完成, **Then** 搜索应该在 1 秒内(50 个文档规模)返回结果 ✅
8. **Given** 搜索结果为空, **When** 用户查看搜索界面, **Then** 应该显示"无匹配结果"的友好提示和图标 ✅
9. **Given** 用户进行搜索, **When** 搜索包含中文、英文、符号, **Then** 搜索应该支持多语言和特殊字符（大小写不敏感） ✅
10. **Given** 搜索结果列表显示, **When** 用户按 ↑↓ 键, **Then** 应该能导航选择结果；按 Enter 键跳转；按 Esc 键关闭搜索 ✅
...
- **FR-030**: 用户必须能够输入关键词搜索所有项目文档 ✅
  - 实现方式：使用 Fuse.js Extended Search 进行精确匹配
  - 快捷键：Cmd+K (Mac) / Ctrl+K (Windows/Linux)
- **FR-031**: 系统必须在搜索结果中显示匹配的文档列表和匹配位置(行号) ✅
  - 显示内容：文档名、相对路径、匹配行号、匹配文本片段（带高亮）
  - 统计信息：文档数、总匹配数
- **FR-032**: 系统必须支持搜索结果的实时过滤(用户输入时更新结果) ✅
  - 防抖延迟：300ms
  - 自动重建索引：文档列表变化时
- **FR-033**: 系统必须在用户点击搜索结果时跳转到对应文档并高亮匹配文本 ✅
  - 智能定位：多个匹配项时定位到目标行最近的匹配
  - 高亮持久化：高亮永久保留，不自动消失
  - Markdown 语法清理：清理任务列表、代码标记、粗体/斜体后进行匹配
- **FR-034**: 系统必须支持多语言搜索(中文、英文、特殊字符) ✅
  - 大小写不敏感
  - 完整 Unicode 支持
- **FR-034.1**: 系统必须支持键盘导航搜索结果 ✅
  - ↑↓ 键选择结果
  - Enter 键确认跳转
  - Esc 键关闭搜索
